...
class InputContainer:
    ...
    persistent_defaults = (
        {}
    )  # some elements need to be remembered without having to create a new instance, for reset button
    ...
    def numerical_widget(
        ...
        if min is None and max is None:
            return dict(
                value=cls.persistent_defaults[key], label=label, step=step
            )
        else:
            return dict(
                value=cls.persistent_defaults[key],
                label=label,
                step=step,
                min=min,
                max=max,
...
class DataInputs(InputContainer):
    ...
    persistent_defaults = dict(voxel_size_z=5, voxel_size_y=2, voxel_size_x=2)
    ...
    def as_cellfinder_arguments(self) -> dict:
        ...
        del data_input_dict[
            "voxel_size_z"
        ]  # doesn't affect self, because asdict creates a copy.
