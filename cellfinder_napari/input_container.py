...
class InputContainer:
    ...
    Intended to be derived to group specific related widget inputs (e.g from
    the same widget section) into a container. Derived classes should be
    Python data classes.
    ...
    @classmethod
    def _custom_widget(
        cls, key: str, custom_label: Optional[str] = None, **kwargs
    ) -> dict:
        """
        Represents a field, given by key, as a formatted widget with the
        field's default value.

        The widget label is the capitalized key by default, with underscores
        replaced by spaces, unless custom_label is specified. Keyword
        arguments like step, min, max, ... are passed to napari underneath.
        """
        label = (
